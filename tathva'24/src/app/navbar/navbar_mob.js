...
const Navbar_mob = () => {
  ...
  return (
    <div className="relative ">
      ...
      <div className="fixed top-4 pt-10 left-4  z-50 cursor-pointer" onClick={toggleNavbar}>
        <div className="space-y-1">
          {isOpen ? (
            // Close (X) icon
            <div className="text-black w-8 h-8 bg-white font-bold text-black flex  justify-center align-middle text-3xl">✕</div>
          ) : (
            // Hamburger icon
            <>
      ...
      <div className={`fixed top-0 left-0 h-screen w-full max-w-full transform ${isOpen ? 'translate-y-0' : '-translate-y-full'} transition-transform duration-500 ease-in-out z-40`}>
        <div 
      ...
      {isOpen && <div className="fixed inset-0 bg-black opacity-50 z-30" onClick={toggleNavbar}></div>}
    </div>
  );
};
